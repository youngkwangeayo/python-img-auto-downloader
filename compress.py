...
class ImageCompressor:
    ...
    def create_zip(self, output_filename=None, compression_level=6):
        ...
        if not self.source_dir.exists():
            print(f"오류: 소스 디렉토리를 찾을 수 없습니다: {self.source_dir}")
            return None
        ...
        print(f"전체 크기: {self.format_size(folder_size)}")
        print(f"압축 레벨: {compression_level}/9")
        ...
        try:
                ...
                for root, dirs, files in os.walk(self.source_dir):
                    for file in files:
                        ...
                        if processed_files % 50 == 0:
                            ...
                            print(f"진행 중: {processed_files}/{file_count} ({progress:.1f}%)")
            ...
            print(f"원본 크기: {self.format_size(folder_size)}")
            print(f"압축 크기: {self.format_size(compressed_size)}")
    ...
    def compress(self, format='zip', output_filename=None, compression_level=6):
        ...
        if format.lower() == 'zip':
            ...
            print(f"지원하지 않는 압축 형식: {format}")
            print("지원 형식: zip, tar.gz")
            return None
